...
/*
 * The following definitions are for has chains and free
 ...
 */
DBHANDLE db_open (const char *pathname, int nhash, int oflag, ...)
{
	DB		*db;
	int		len, mode;
	size_t	i;
	char	asciiptr[PTR_SZ + 1],
			hash[(nhash + 1) * PTR_SZ + 2]; /* + 2 for newline and null */
	...
	db->nhash		= nhash;	/* hash table size */
	...
	if ((oflag & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC)) {
		...
		if (statbuff.st_size == 0) {
			/*
			 * We have to build a list of (nhash + 1) chain
...
static void _db_dodelete (DB *db)
{
	...
	*ptr = 0;	/* null terminate for _db_writedat */
	...
	/*
	 * Rewrite the chain ptr that pointed to this record beging
...
static void _db_writedat (DB *db, const char *data, off_t offset, int whence)
{
	 ...
	 */
	if (whence == SEEK_END)	{	/* we're appeing, lock entire file */
...
char *db_nextrec (DBHANDLE h, char *key)
{
	 ...
	 * a record in the middle of its beging deleted.
	 */
